...
class indexer:
    ...
    def root(self):
        try:
            url = base64.b64decode('aHR0cDovL2V5ZWNhbmR5dHYudWsvdHZleWUvTWFpbi9ob21lLnhtbA==')
            self.list = self.eyecandy_list(url)
    ...
    def getx(self, url, worker=False):
        try:
            self.list = self.eyecandy_list('', result=url)
    ...
    def addSearch(self, url=None):
        try:
            link = base64.b64decode('aHR0cDovL2V5ZWNhbmR5dHYudWsvdHZleWUvTWFpbi9FeWVDYW5keS9TZWFyY2gvc2VhcmNoLnhtbA==')
            ...
            self.list = [] ; threads = [] 
            for link in links: threads.append(workers.Thread(self.eyecandy_list, link))
            ...
            for i in self.list:
                try:
                    name = ''
                    if not i['vip'] in ['EyeCandy TV']: name += '[B]%s[/B] | ' % i['vip'].upper()
                    name += i['name']
                    i.update({'name' : name})
                except:
                    pass
    ...
    def eyecandy_list(self, url, result=None):
        try:
