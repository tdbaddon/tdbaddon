...
class indexer:
    ...
    def root(self):
        try:
            url = 'https://raw.githubusercontent.com/iCanuck/NHLstreams/master/NHL.xml'
            self.list = self.nhlstreams_list(url)
    ...
    def getx(self, url, worker=False):
        try:
            self.list = self.nhlstreams_list('', result=url)
    ...
    def addSearch(self, url=None):
        try:
            link = 'https://raw.githubusercontent.com/iCanuck/NHLstreams/master/search.xml'
            ...
            self.list = [] ; threads = [] 
            for link in links: threads.append(workers.Thread(self.nhlstreams_list, link))
    ...
    def nhlstreams_list(self, url, result=None):
        try:
