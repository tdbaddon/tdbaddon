...
class source:
	...
	def movie(self, imdb, title, year):
		...
		try:
			...
			r = BeautifulSoup(requests.get(query, headers=headers, timeout=15).content)
			r = r.findAll('iframe')
            # print ("ANIMETOON s1",  r)
			for u in r:
				u = u['src'].encode('utf-8')
				if u.startswith("//"): u = "http:" + u
				print("WONLINE PASSED", u)
				if not "youtube" in u and not "openload" in u: self.zen_url.append(u)
		...
		try:
			...
			for url in self.zen_url:
				...
				try:
					...
					r = BeautifulSoup(requests.get(url, headers=headers, timeout=15).content)
					r = r.findAll('script')
					for content in r:
						if jsunpack.detect(content.text):
							unpacked = jsunpack.unpack(content.text)
							unpacked_script.append(unpacked)
						try:
							match = re.compile('file:\s*"(.+?)",label:"(.+?)",').findall(content.text)
					...
					pass
			for content2 in unpacked_script:
				try:
					match = re.compile('file":"(.+?)","label":"(.+?)",').findall(content2)
					for href, quality in match:
						href = href.encode('utf-8')
						href = href.replace('\\','')
						print("WONLINE UNPACKED SCRIPTS", quality)
						quality = quality_tag(quality)
						sources.append({'source': 'gvideo', 'quality':quality, 'provider': 'Onlinemovies', 'url': href, 'direct': True, 'debridonly': False})
				except:
					pass
