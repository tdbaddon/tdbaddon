					...
					for movie_links in file_links:
						...
						if year in movie_links:
							if cleanmovie in cleantitle.get(movie_links):
								...
								self.zen_url.append([movielink,movielink])
				except:
					pass


	 
			# print ("HEYDL PASSED ITEMS", self.zen_url)
			return self.zen_url
        except:
            return
			...
			for movielink,title in self.zen_url:
						...
						if not any(value in url for value in ['uploadkadeh','wordpress','crazy4tv','imdb.com','youtube','userboard','kumpulbagi','mexashare','myvideolink.xyz', 'myvideolinks.xyz' , 'costaction', 'crazydl','.rar', '.RAR',  'safelinking','linx.2ddl.ag','upload.so','.zip', 'go4up', 'adf.ly','.jpg','.jpeg']):
							if "http" in url:
								...
								sources.append({'source': 'gvideo', 'quality': quality, 'provider': 'Heydl', 'url': url, 'info': info,'direct': True, 'debridonly': False})
