...
#Default veriables
AddonTitle     = "[COLOR deepskyblue][B]Porn[/COLOR][COLOR crimson]HD[/B][/COLOR]"
addon_id       = 'plugin.video.pornhd'
...
def MAIN_MENU():
	...
	addDir("[COLOR crimson][B]SEARCH[/B][/COLOR]","url",2,search_icon,fanart,'')

	result = requests.get('http://www.pornhd.com/category')
	match = re.compile('<div class="tag-150-container">(.+?)<div class="footer-zone">',re.DOTALL).findall(result.content)
	string = str(match)
	...
	for item in match2:
		...
		name = "[COLOR deepskyblue][B]" + title + "[/COLOR][/B]"
		name = CLEANUP(name)
		url = 'http://pornhd.com' + url
		addDir(name,url,1,icon_cat,fanart,'')

	addItem("[COLOR crimson][B]Twitter Support: @echo_coding[/B][/COLOR]","url",999,twitter_icon,fanart,'')
...
def GET_CONTENT(url):
	...
	match = re.compile('<section id="pageContent" class="page-content " >(.+?)<div class="pager paging">',re.DOTALL).findall(result.content)
	string = str(match)
	...
	for item in match2:
		...
		name = "[COLOR deepskyblue][B]" + title + "[/B][/COLOR]"
		name = CLEANUP(name)
	...
	try:
		...
		addDir('[COLOR crimson]Next Page >>[/COLOR]',url,1,next_icon,fanart,'')       
	except:pass
...
def GET_LUCKY():
	...
	if lucky == 1:
		dp.create(AddonTitle,"[B][COLOR crimson]Please wait.[/B][/COLOR]",'[B][COLOR deepskyblue]We are getting the moisturiser.[/B][/COLOR]','[B][COLOR azure]Do you have the wipes ready?[/B][/COLOR]' )
	elif lucky == 2:
		dp.create(AddonTitle,"[B][COLOR crimson]Please wait.[/B][/COLOR]",'[B][COLOR deepskyblue]I am just taking off my pants.[/B][/COLOR]','[B][COLOR azure]Darn belt![/B][/COLOR]' )
	elif lucky == 3:
		dp.create(AddonTitle,"[B][COLOR crimson]Please wait.[/B][/COLOR]",'[B][COLOR deepskyblue]Are the curtains closed?[/B][/COLOR]','[B][COLOR azure]Oh baby its cold outside![/B][/COLOR]' )
	elif lucky == 4:
		dp.create(AddonTitle,"[B][COLOR crimson]Please wait.[/B][/COLOR]",'[B][COLOR deepskyblue]This is my fifth time today.[/B][/COLOR]','[B][COLOR azure]How about you?[/B][/COLOR]' )
	elif lucky == 5:
		dp.create(AddonTitle,"[B][COLOR crimson]Please wait.[/B][/COLOR]",'[B][COLOR deepskyblue]Please no buffer, please no buffer![/B][/COLOR]')
	elif lucky == 6:
		dp.create(AddonTitle,"[B][COLOR crimson]Please wait.[/B][/COLOR]",'[B][COLOR deepskyblue]I think I am going blind :-/[/B][/COLOR]','[B][COLOR azure]Oh no, just something in my eye.[/B][/COLOR]' )
	elif lucky == 7:
		dp.create(AddonTitle,"[B][COLOR crimson]Please wait.[/B][/COLOR]",'[B][COLOR deepskyblue]Did I turn the oven off?[/B][/COLOR]','[B][COLOR azure]It can wait![/B][/COLOR]' )
	elif lucky == 8:
		dp.create(AddonTitle,"[B][COLOR crimson]Please wait.[/B][/COLOR]",'[B][COLOR deepskyblue]Your video is coming. Are you?[/B][/COLOR]','[B][COLOR azure]Do you get it?[/B][/COLOR]' )
	elif lucky == 9:
		dp.create(AddonTitle,"[B][COLOR crimson]Please wait.[/B][/COLOR]",'[B][COLOR deepskyblue]Kodi does not save your browsing history :-D[/B][/COLOR]','[B][COLOR azure]Thats lucky isnt it :-)[/B][/COLOR]' )
	else:
		dp.create(AddonTitle,"[B][COLOR crimson]Please wait.[/B][/COLOR]",'[B][COLOR deepskyblue]There are more XXX addons by ECHO.[/B][/COLOR]','[B][COLOR azure]Just so you know.[/B][/COLOR]' )
