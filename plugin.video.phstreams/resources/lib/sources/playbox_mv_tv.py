...
class source:
    def __init__(self):
        ...
        self.search_link = '/api/box?type=search&os=Android&v=2.0.2&k=0&keyword=%s'
        self.detail_link = '/api/box?type=detail&os=Android&v=2.0.2&k=0&id=%s'
        self.stream_link = '/api/box?type=stream&os=Android&v=2.0.2&k=0&id=%s'
    ...
    def get_show(self, imdb, tvdb, tvshowtitle, year):
        try:
            ...
            years = ['%s' % str(year), '%s' % str(int(year)+1), '%s' % str(int(year)-1)]
            result = [(i['id'], i['title'].encode('utf-8')) for i in result]
            result = [i for i in result if tvshowtitle == cleantitle.tv(i[1])][:2]
            result = [(i[0], self.base_link + self.detail_link % i[0]) for i in result]
            result = [(i[0], client.source(i[1])) for i in result]
            result = [(i[0], json.loads(i[1])['data']['state']) for i in result]
            result = [i[0] for i in result if any(x in i[1] for x in years)][0]
