...
class source:
    ...
    def movie(self, imdb, title, year):
		...
		try:
			...
			print ("MOVIES PASSED MYVIDEOLINK", self.genesisreborn_url)
    ...
    def episode(self, url, imdb, tvdb, title, premiered, season, episode):
        ...
        try:
			...
			u = [(i[0].encode('utf-8'), i[1].encode('utf-8'), type) for i in r]
			self.genesisreborn_url += u
			print ("MYVIDEOLINK SHOWS", self.genesisreborn_url)
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
			sources = []
			for url,title,type in self.genesisreborn_url:
