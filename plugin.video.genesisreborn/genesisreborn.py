...
if action == None:
    ...
    from resources.lib.modules import control
    dialog = xbmcgui.Dialog()
    USERDATA     =  xbmc.translatePath(os.path.join('special://home/userdata/addon_data/plugin.video.genesisreborn',''))
    if os.path.exists(os.path.join(USERDATA,'favourites.db')):
		...
		if not backupdir == '':
		   ...
		   backup_ui_zip = xbmc.translatePath(os.path.join(backupdir,'genesisreborn_watchlist.zip'))
