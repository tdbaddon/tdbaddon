...
class source:
    ...
    def movie(self, imdb, title, localtitle, year):
        try:
            r = self.searchMovie(title, year)

            if r == None:
                t = cache.get(self.getOriginalTitle, 900, imdb)
                if t != title:
                    r = self.searchMovie(t, year)

            return urllib.urlencode({'url': r['url'], 'episode': 0, 'headers': r['headers']})
        except:
            return
    ...
    def episode(self, url, imdb, tvdb, title, premiered, season, episode):
        try:
            ...
            return urllib.urlencode({'url': r['url'], 'episode': episode, 'headers': r['headers']})
        except:
            return
