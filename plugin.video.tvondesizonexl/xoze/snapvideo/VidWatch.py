...
def retrieveVideoInfo(video_id):
    ...
    try:
        video_info_link = 'http://vidwatch3.me/embed-' + str(video_id) + '-540x304.html'
        ...
        if final_video_link == '' or final_video_link == None:
            paramSet = re.compile("returnp\}\(\"(.+?)\",(\d+),(\d+),\"(.+?)\"").findall(html)
            try:
                if len(paramSet) > 0:
