...
addon = xbmcaddon.Addon('TRC4.Albdroid-IPTV.AL')
...
source_file = os.path.join(home, 'jscryptor.py')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[TRC4.Albdroid-IPTV.AL-%s]: %s" %(addon_version, string))


def makeRequest(url, headers=None):
        try:
            if headers is None:
                headers = {'User-agent' : 'Samsung'} #Mozilla/5.0 (Windows NT 6.1; WOW64; rv:19.0) Gecko/20100101 Firefox/19.0
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(Albdroid IPTV,[COLOR lime]We failed with error code - [/COLOR]"+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(Albdroid IPTV,[COLOR lime]We failed to reach a server. - [/COLOR]"+str(e.reason)+",10000,"+icon+")")
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(Albdroid IPTV,New source added.,5000,"+icon+")")
        if not url is None:
...
def playmediawithproxy(media_url, name, iconImage,proxyip,port, proxyuser=None, proxypass=None): #jairox
    ...
    progress.create('Progress', '[COLOR=lime]Playing With Custom Proxy[/COLOR]')
    progress.update( 10, "", "[COLOR=lime]Setting Proxy..[/COLOR]", "" )
    proxyset=False
    existing_proxy=''
    ...
    try:
        ...
        proxyset=True
        progress.update( 80, "", "[COLOR=lime]Setting Proxy Complete, Now Playing[/COLOR]", "" )
...
def createM3uForDash(url,useragent=None):
    ...
    source_file = os.path.join(profile, 'TRC4.m3u')
    str+='\n'
...
def addFavorite(name,url,iconimage,fanart,mode,playlist=None,regexs=None):
        ...
        if os.path.exists(favorites)==False:
            addon_log('[COLOR=lime]Making Favorites File[/COLOR]')
            favList.append((name,url,iconimage,fanart,mode,playlist,regexs))
            a = open(favorites, "w")
            a.write(json.dumps(favList))
            a.close()
        else:
            addon_log('[COLOR=lime]Appending Favorites[/COLOR]')
...
def urlsolver(url):
    ...
    if host:
        ...
        xbmc.executebuiltin("XBMC.Notification(Albdroid IPTV,Url Resolver donot Support This Domain. - ,5000)")
        resolver=url
    return resolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('[COLOR=lime]Albdroid IPTV[/COLOR]','[COLOR=lime]Choose a Location to Save Files.[/COLOR]',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('[COLOR=lime]Albdroid IPTV[/COLOR]', '[COLOR=lime]Do You Want to Add This File as a Source?[/COLOR]')
        if ret:
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':
                ...
                if name in str(SOURCES):
                    contextMenu.append(('[COLOR=lime]Remove from Sources[/COLOR]','XBMC.RunPlugin(%s?mode=8&name=%s)' %(sys.argv[0], urllib.quote_plus(name))))
                ...
                contextMenu.append(('[COLOR=lime]Remove from Albdroid IPTV Favorites[/COLOR]','XBMC.RunPlugin(%s?mode=6&name=%s)'
            ...
            if showcontext == '!!update':
                ...
                contextMenu.append(('[COLOR lime]Update[/COLOR]','XBMC.RunPlugin(%s)' %fav_params2))
            if not name in FAV:
                contextMenu.append(('[COLOR=lime]Add to Albdroid IPTV Favorites[/COLOR]','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def ytdl_download(url,title,media_type='video'):
    ...
    if not url == '':
        ...
        xbmc.executebuiltin("XBMC.Notification(DOWNLOAD,[COLOR lime]First Play While Playing Download[/COLOR] ,10000)")
...
#                print 'setting as folder in link'
            contextMenu.append(('[COLOR lime]Download Currently Playing[/COLOR]','XBMC.RunPlugin(%s?url=%s&mode=21&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(url), urllib.quote_plus(name))))
            ...
            mode = '18'
            contextMenu.append(('[COLOR lime]Download[/COLOR]','XBMC.RunPlugin(%s?url=%s&mode=23&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(url), urllib.quote_plus(name))))
            if addon.getSetting('dlaudioonly') == 'true':
                contextMenu.append(('!!Download [COLOR lime]Audio[/COLOR]','XBMC.RunPlugin(%s?url=%s&mode=24&name=%s)'
                                        %(sys.argv[0], urllib.quote_plus(url), urllib.quote_plus(name))))
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('[COLOR=lime]Remove From Albdroid IPTV Favorites[/COLOR]','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('[COLOR=lime]Add to Albdroid IPTV Favorites[/COLOR]','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
#            ln+='</item>'
        ...
        if url:
            ...
            xbmc.executebuiltin("XBMC.Notification(Albdroid IPTV,Failed to Extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(Albdroid IPTV,[COLOR lime]Please Install YouTube-dl[/COLOR] module ,10000,"")")
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification(Albdroid IPTV,Parental Block Disabled,5000,"+icon+")")
        else:
            xbmc.executebuiltin("XBMC.Notification(Albdroid IPTV,Wrong Pin??,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
elif mode==56:
    addon_log("disable lock")
    addon.setSetting('parentalblocked', "true")
    xbmc.executebuiltin("XBMC.Notification(Albdroid IPTV,Parental block enabled,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
