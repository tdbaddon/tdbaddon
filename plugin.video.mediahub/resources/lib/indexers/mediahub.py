...
class indexer:
    ...
    def root(self):
        try:
            url = base64.b64decode ('aHR0cDovL21oLmNvbWxpLmNvbS9MSVNUUy9Ib21lLlhNTA==')
            self.list = self.mediahub_list(url)
    ...
    def getx(self, url):
        try:
            self.list = self.mediahub_list('', result=url)
            self.addDirectory(self.list)
            return self.list
        except:
            pass
    ...
    def addSearch(self, url=None):
        try:
            ...
            self.list = [] ; threads = [] 
            for link in links: threads.append(workers.Thread(self.mediahub_list, link))
            ...
            for i in self.list:
                try:
                    name = ''
                    if not i['vip'] in ['MediaHub TV']: name += '[B]%s[/B] | ' % i['vip'].upper()
                    name += i['name']
                    i.update({'name' : name})
                except:
                    pass
    ...
    def mediahub_list(self, url, result=None):
        try:
