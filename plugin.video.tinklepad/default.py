...
import urllib,urllib2,re,os,threading,datetime,time,base64,xbmc,xbmcplugin,xbmcgui,xbmcaddon,xbmcvfs,array,random,urlparse,json
...
addonTitles         = os.path.join(addonPath,'resources/art/Title.png')
        ...
        if imdb == '0': 
            try:
                if self.content == 'movie':
                    imdb = metaget.get_meta('movie', self.title, year=str(self.year))['imdb_id']
                else:
                    imdb = metaget.get_meta('tvshow', self.title)['imdb_id']
            except:
                pass
...
class index:
    def getUserInput(self, header, userInput=''):
        k = control.keyboard(userInput, header) ; k.doModal()
        q = k.getText() if k.isConfirmed() else None

        if (q == None or q == ''): 
            self.infoDialog('No query submitted')
            quit()
        else: return q
    ...
    def pageList(self, pageList):
        ...
        for i in pageList:
            try:
                ...
                if action.startswith("tvshows") or action.endswith("tvshows"):
                    u = '%s?action=tvshows&url=%s' % (sys.argv[0], sysurl)
                else:
    ...
    def nextList(self, nextList, hd=0):
        ...
        if action.startswith("tvshows") or action.endswith("tvshows"):
            ...
            u = '%s?action=movies&hd=1&url=%s' % (sys.argv[0], sysurl)
        else:
            u = '%s?action=movies&url=%s' % (sys.argv[0], sysurl)
    ...
    def movieList(self, movieList, hd=0):
        if movieList == None: return

        if action == 'episodes' or action == 'seasons': getmeta = 'false'
        else: getmeta = getSetting("meta")
        ...
        for i in movieList:
            try:
                ...
                if getmeta == 'true':
                    if action.startswith("tvshows") or action.endswith("tvshows"): meta = metaget.get_meta('tvshow', name=title)
                    else: meta = metaget.get_meta('movie', name=title, year=year)
                    ...
                    if int(meta["year"]) == int(year):
                        ...
                        if trailer == '': trailer = 'tt%s' % imdb.replace("tt", '')
                        if poster == '': 
                            poster = image
                            meta.update({"poster": image})
                    else:
                        meta = {'label': name, 'title': title, 'year': year, 'imdb_id' : imdb, 'genre' : genre, 'plot': plot, 'poster': image}
                        trailer, poster = 'tt%s' % imdb.replace("tt", ''), image
                else:
                    meta = {'label': name, 'title': title, 'year': year, 'imdb_id' : imdb, 'genre' : genre, 'plot': plot, 'poster': image}
                    trailer, poster = 'tt%s' % imdb, image
                if getmeta == 'true' and getSetting("fanart") == 'true' and yearMatch == True:
                ...
                if hd == 1:
                    ...
                    u = 'plugin://plugin.video.exodus/?action=play&title=%s&year=%s&meta=%s' % (systitle, year, sysmeta)
                elif re.match(r'Season \d+$', title):
                    setPlayable = "false"
                    setFolder = True
                    u = '%s?action=episodes&url=%s' % (sys.argv[0], sysurl)
                elif action.startswith("tvshows") or action.endswith("tvshows"):
                    setPlayable = "false"
                    setFolder = True
                ...
                xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]),url=u,listitem=item,totalItems=total,isFolder=setFolder)
            except:
                pass
...
class contextMenu:
    ...
    def download(self, name, url, type):
        try:
            ...
            ext = url.rsplit('/', 1)[-1].rsplit('?', 1)[0].rsplit('|', 1)[0].strip().lower()
            valid_exts = ["m4v ", "3gp", "nsv", "ts", "ty", "strm", "rm", "rmvb", "m3u", "ifo", "mov", "qt", "divx", "xvid", "bivx", "vob", "nrg", "img", "iso", "pva", "wmv", "asf", "asx", "ogm", "m2v", "avi", "bin", "dat", "dvr-ms", "mpg", "mpeg", "mp4", "mkv", "avc", "vp3", "svq3", "nuv", "viv", "dv", "fli", "flv", "rar", "001", "wpl", "zip"]
            if not ext in valid_exts: ext = 'mp4'
...
class root:
    ...
    def get(self):
        self.rootList.append({'name': 30521, 'image': 'movies.png', 'action': 'root_movies'})
        self.rootList.append({'name': 30522, 'image': 'tv.png', 'action': 'root_tvshows'})
        index().rootList(self.rootList)
        index().downloadList()
        
    def getMovies(self):
        '''if showHD:
            self.rootList.append({'name': 30500, 'image': 'hd.png', 'action': 'movies_hd'})'''
        self.rootList.append({'name': 30500, 'image': 'hd.png', 'action': 'movies_hd'})
        self.rootList.append({'name': 30520, 'image': 'ts.png', 'action': 'movies_ts'})
        self.rootList.append({'name': 30501, 'image': 'Title.png', 'action': 'titles_movies'})
        #self.rootList.append({'name': 30502, 'image': 'IMDb.png', 'action': 'movies_imdb'})
        self.rootList.append({'name': 30503, 'image': 'Release.png', 'action': 'movies_release'})
        self.rootList.append({'name': 30504, 'image': 'Added.png', 'action': 'movies_added'})
        self.rootList.append({'name': 30505, 'image': 'Rating.png', 'action': 'movies_rating'})
        self.rootList.append({'name': 30506, 'image': 'Views.png', 'action': 'movies_views'})
        self.rootList.append({'name': 30515, 'image': 'Featured.png', 'action': 'movies_featured'})
        self.rootList.append({'name': 30516, 'image': 'Trending.png', 'action': 'movies_views_today'})
        self.rootList.append({'name': 30507, 'image': 'Genres.png', 'action': 'genres_movies'})
        self.rootList.append({'name': 30508, 'image': 'Years.png', 'action': 'years_movies'})
        self.rootList.append({'name': 30517, 'image': 'People.png', 'action': 'people_movies'})
        #self.rootList.append({'name': 30509, 'image': 'Languages.png', 'action': 'languages_movies'})
        self.rootList.append({'name': 30510, 'image': 'Favourites.png', 'action': 'movies_favourites'})
        self.rootList.append({'name': 30511, 'image': 'Search.png', 'action': 'movies_search'})
        index().rootList(self.rootList)
        index().downloadList()
        
    def getShows(self):
        self.rootList.append({'name': 30501, 'image': 'Title.png', 'action': 'titles_tvshows'})
        self.rootList.append({'name': 30504, 'image': 'Added.png', 'action': 'tvshows_added'})
        self.rootList.append({'name': 30506, 'image': 'Views.png', 'action': 'tvshows_views'})
        self.rootList.append({'name': 30516, 'image': 'Trending.png', 'action': 'tvshows_views_today'})
        self.rootList.append({'name': 30507, 'image': 'Genres.png', 'action': 'genres_tvshows'})
        self.rootList.append({'name': 30510, 'image': 'Favourites.png', 'action': 'tvshows_favourites'})
        self.rootList.append({'name': 30511, 'image': 'Search.png', 'action': 'tvshows_search'})
        index().rootList(self.rootList)
        index().downloadList()
...
class link:
    def __init__(self):
        ...
        if action.startswith("tvshows") or action.endswith("tvshows"): self.tinkle_base = self.tinkle_base + '/tv'
        self.tinkle_hd = self.tinkle_base + '/latest-hd'
        self.tinkle_ts = self.tinkle_base + '/latest-ts'
        #self.tinkle_ajax = self.tinkle_base + '/wp-admin/admin-ajax.php'
        #self.tinkle_post = '?meta_key=imdbRating&orderby=meta_value&order=desc'
        self.tinkle_title = self.tinkle_base
        self.tinkle_release = self.tinkle_base + '/new-release'
        #self.tinkle_imdb = self.tinkle_base + '/files/movies/?meta_key=imdbRating&orderby=meta_value&order=desc'
        self.tinkle_added = self.tinkle_base + '/latest-added'
        self.tinkle_rating = self.tinkle_base + '/top-rated'
        self.tinkle_views = self.tinkle_base + '/most-popular'
        self.tinkle_featured = self.tinkle_base + '/featured'
        self.tinkle_views_today = self.tinkle_base + '/popular-today'
        self.tinkle_genre = self.tinkle_base + '/genres'
        self.tinkle_year = self.tinkle_base + '/year'
        self.tinkle_language = self.tinkle_base + '/languages'
        self.tinkle_search = 'http://5movies.to/search.php'
        self.tinkle_people = self.tinkle_base + '/%s/%s'
...
class people:
    ...
    def root(self):
        rootList = []
        rootList.append({'name': 30518, 'image': 'Male.png', 'action': 'people_male'})
        rootList.append({'name': 30519, 'image': 'Female.png', 'action': 'people_female'})
        rootList.append({'name': 30511, 'image': 'Search.png', 'action': 'people_search'})
        index().rootList(rootList)

    def get(self, gender="male,female"):
        #self.list = self.tinkle_list(gender)
        self.list = cache.get(self.tinkle_list, 24, gender)
        #self.list = sorted(self.list, key=itemgetter('name'))
        index().pageList(self.list)
    
    def search(self, query=None):
        if query is None:
            self.query = index().getUserInput(language(30362).encode("utf-8"), '')
        else:
            self.query = query
        if not (self.query is None or self.query == ''):
            self.query = link().imdb_search + urllib.quote_plus(self.query)
            self.list = self.tinkle_list(self.query)
            index().pageList(self.list)

    def tinkle_list(self, gender):
        try:
            result = client.request(link().imdb_people % gender)
            #result = client.parseDOM(result, "table", attrs = { "class": "results" })[0]
            result = re.compile('(<tr class="even detailed">.*?)</table>', re.DOTALL).findall(result)[0]
            actors = client.parseDOM(result, "tr")
        except:
            return

        for actor in actors:
            try:
                role = client.parseDOM(actor, "span", attrs = { "class": "description" })[0]
                role = re.search('([a-zA-Z]+)', role).groups()[0]
                role = client.replaceHTMLCodes(role)
                role = role.encode('utf-8')
                #print role
                
                type1 = ["Actor", "Actress", "Self"]
                type2 = ["Writer", "Director", "Producer"]
                if role in type1:
                    directory = "actor"
                elif role in type2:
                    directory = "director"
                ...
                import unicodedata
                name = client.parseDOM(actor, "a")[1]
                name = name.strip()
                name = client.replaceHTMLCodes(name)
                enc_name = ''.join((c for c in unicodedata.normalize('NFD', name) if unicodedata.category(c) != 'Mn'))
                #name = "%s (%s)" % (name, role)
                name = name.encode('utf-8')
                #print name
                
                enc_name = re.sub('[\.\']', '', enc_name)
                enc_name = re.sub('[^a-zA-Z\s]', ' ', enc_name)
                enc_name = enc_name.replace(" ", "_")
                #print enc_name

                url = '%s/%s/%s' % (link().tinkle_base, directory, enc_name)
                url = client.replaceHTMLCodes(url)
                url = url.encode('utf-8')
                #print url

                image = client.parseDOM(actor, "img", ret="src")[0]
                image = client.replaceHTMLCodes(image)
                image = image.encode('utf-8')
                image = image.replace("SX54", "SX216")
                image = image.replace("SY74", "SY296")
                image = image.replace("54,74", "216,296")
...
class titles:
    ...
    def get(self):
        self.list = self.tinkle_list()
        #self.list = cache.get(self.tinkle_list, 72)
        #self.list = sorted(self.list, key=itemgetter('name'))
        index().pageList(self.list)

    def tinkle_list(self):
        try:
            titles = ['0-9', 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z']
        except:
            return
        ...
        for i in titles:
            try:
                ...
                url = '%s/%s/' % (link().tinkle_base, i)
                url = client.replaceHTMLCodes(url)
                url = url.encode('utf-8')
                #print url

                image = addonTitles.encode('utf-8')
...
class genres:
    ...
    def tinkle_list(self):
        try:
            if action.endswith("tvshows"):
                genres = ["Action", "Adventure", "Animation", "Biography", "Comedy", "Crime", "Documentary", "Drama", "Family", "Fantasy", "Game-Show", "History", "Horror", "Japanese", "Korean", "Music", "Musical", "Mystery", "Reality-TV", "Romance", "Sci-Fi", "Short", "Sport", "Talk-Show", "Thriller", "War", "Western"]
            else: 
        ...
        for genre in genres:
            try:
                ...
                url = '%s/%s' % (link().tinkle_base, genre.lower())
                url = client.replaceHTMLCodes(url)
                url = url.encode('utf-8')
                #print url
...
class languages:
    ...
    def tinkle_list(self):
        try:
            result = client.request(link().tinkle_language, timeout='30')
        ...
        for language in languages:
            try:
                ...
                url = '%s%s%s' % (link().tinkle_base, url, link().tinkle_post)
                url = client.replaceHTMLCodes(url)
                url = url.encode('utf-8')
...
class movies:
    ...
    def get(self, url, hd):
        #self.list = self.tinkle_list(url)
        self.list = cache.get(self.tinkle_list, 1, url)
        index().movieList(self.list, hd)
        index().nextList(self.list, hd)

    def hd_movies(self):
        if xbmc.getCondVisibility('System.HasAddon(%s)' % exodusName) and getSetting('useExodus') == 'true':
            hd = 1
        else:
            hd = 0
        #self.list = self.tinkle_list(link().tinkle_hd)
    ...
    def title(self):
        ...
        self.list = cache.get(self.tinkle_list, 1, link().tinkle_title)
        index().movieList(self.list)
        index().nextList(self.list)

    def release(self):
        #self.list = self.tinkle_list(link().tinkle_release)
        self.list = cache.get(self.tinkle_list, 1, link().tinkle_release)
        index().movieList(self.list)
        index().nextList(self.list)

    def imdb(self):
        #self.list = self.tinkle_list(link().tinkle_imdb)
        self.list = cache.get(self.tinkle_list, 1, link().tinkle_imdb)
        index().movieList(self.list)
        index().nextList(self.list)

    def added(self):
        #self.list = self.tinkle_list(link().tinkle_added)
        self.list = cache.get(self.tinkle_list, 1, link().tinkle_added)
        index().movieList(self.list)
        index().nextList(self.list)

    def rating(self):
        #self.list = self.tinkle_list(link().tinkle_rating)
        self.list = cache.get(self.tinkle_list, 1, link().tinkle_rating)
        index().movieList(self.list)
        index().nextList(self.list)

    def views(self):
        #self.list = self.tinkle_list(link().tinkle_views)
        self.list = cache.get(self.tinkle_list, 1, link().tinkle_views)
        index().movieList(self.list)
        index().nextList(self.list)
    
    def featured(self):
        #self.list = self.tinkle_list(link().tinkle_rating)
        self.list = cache.get(self.tinkle_list, 1, link().tinkle_featured)
        index().movieList(self.list)
        index().nextList(self.list)

    def views_today(self):
        #self.list = self.tinkle_list(link().tinkle_views)
        self.list = cache.get(self.tinkle_list, 1, link().tinkle_views_today)
        index().movieList(self.list)
        index().nextList(self.list)
    ...
    def search(self, query=None):
        if query is None:
            ...
            self.query = query
        if not (self.query is None or self.query == ''):
            self.query = link().tinkle_search + '?q=' + urllib.quote_plus(self.query)
            self.list = self.tinkle_list(self.query)
            index().movieList(self.list)
            index().nextList(self.list)
    ...
    def tinkle_list(self, url, post=None):
        ...
        try:
            next = re.search("<a href='(\S+)(?='>Next</a>)", result)
            next = next.groups()[0]
            if next.startswith('/'): 
                next = '%s%s' % ('http://5movies.to', next)
            else:
                next = '%s/%s' % ('http://5movies.to', next)
        ...
        for movie in movies:
            try:
                fullName = client.parseDOM(movie, "img", ret="alt")[0]
                fullName = re.search('((.+?)\s\((\d+)\))', fullName)
                
                name = client.replaceHTMLCodes(fullName.groups()[0])
                name = name.encode('utf-8')
                #print name
                
                title = fullName.groups()[1]
                title = client.replaceHTMLCodes(title)
                title = title.encode('utf-8')
                #print title

                year = fullName.groups()[2]
    ...
    def tinkle_list2(self, query):
        try:
            result = client.request(link().tinkle_ajax, post=query, timeout='30')
...
class seasons:
    ...
    def tinkle_list(self, url, post=None):
        try:
            ...
            plot = client.parseDOM(result, "div", attrs = { "id": "plot" })[0]
            plot = client.replaceHTMLCodes(plot)
            plot = plot.encode('utf-8')
            #print ":::p", plot
            
            img = client.parseDOM(result, "div", attrs = { "class": "view-img" })[0]
            image = client.parseDOM(img, "img", ret="src")[0]
            image = client.replaceHTMLCodes(image)
            if image.startswith("//"): image = "http:%s" % image
            image = image.encode('utf-8')
            #print ":::i", image
            
            fullName = client.parseDOM(img, "img", ret="alt")[0]
            fullName = re.search('((.+?)\s\((\d+)\))', fullName)
            ...
            year = client.replaceHTMLCodes(fullName.groups()[2])
            year = year.encode('utf-8')
            #print ":::y", year
            
            try:
                imdb = re.search(r'(?://|\.)imdb\.com/title/(tt\d+)', result).groups()[0]
                imdb = imdb.encode('utf-8')
            except:
                imdb = '0'
            
            try:
                genre = re.search(r'<tr>\s*<td width=[\"\']60[\"\']>Genre:</td>\s*<td>(.+)</td>\s*</tr>', result).groups()[0]
                genre = re.sub('<.*?>', '', genre)
                genre = [i.strip() for i in genre.split('|')]
                genre = " / ".join(genre)
                genre = client.replaceHTMLCodes(genre)
            ...
            result = client.parseDOM(result, "div", attrs = { "class": "Season" })[0]
            seasons = re.findall(r'(<a.*?class=[\'\"]season-toggle[\'\"].*?</a>)', result)
        ...
        for season in seasons:
            try:
                title = client.parseDOM(season, "a")[0]
                title = re.sub(r'&.+?;', '', title).strip()
                title = client.replaceHTMLCodes(title)
                title = title.encode('utf-8')
                
                url = client.parseDOM(season, "a", ret="href")[0]
                url = client.replaceHTMLCodes(url)
                if url.startswith("//"): url = "http:%s" % url
                url = url.encode('utf-8')

                self.list.append({'name': name, 'url': url, 'image': image, 'title': title, 'year': year, 'imdb': imdb, 'genre': genre, 'plot': plot, 'next': ''})
            except:
                pass
...
class episodes:
    ...
    def tinkle_list(self, url, post=None):
        try:
            ...
            result = client.parseDOM(result, "div", attrs = { "class": "Episode" })[0]
            episodes = re.findall(r'(<a.+?</a>)', result)
        ...
        for episode in episodes:
            try:
                title = client.parseDOM(episode, "a")[0]
                title = re.sub(r'&.+?;', '', title).strip()
                title = re.sub(r'<.+?>', '', title).strip()
                title = client.replaceHTMLCodes(title)
                title = title.encode('utf-8')
                ...
                try:
                    name = client.parseDOM(episode, "a", ret="title")[0]
                    name = client.replaceHTMLCodes(name)
                    name = name.encode('utf-8')
                except:
                    name = ''

                url = client.parseDOM(episode, "a", ret="href")[0]
                url = client.replaceHTMLCodes(url)
                if url.startswith("//"): url = "http:%s" % url
                url = url.encode('utf-8')
...
class trailer:
    def __init__(self):
        ...
        self.youtube_query = '%s/results?sp=EgQQARgB&q=' % self.youtube_base
        self.trakt_query = 'https://trakt.tv/search/imdb?query='
        self.youtube_watch = '%s/watch?v=' % self.youtube_base
    ...
    def run(self, name, url):
        try:
            ...
            if url.startswith(self.youtube_watch):
                url = self.youtube(url)
                if url is False: raise Exception()
                return url
            elif url.startswith('tt'):
                url = '%s%s' % (self.trakt_query, url)
                url = self.trakt_search(url)
                if url is None: url = self.youtube_search(name)
                url = self.youtube(url)
                if url is False: raise Exception()
                return url
            else:
                return 'plugin://plugin.video.youtube/play/?video_id=%s' % url
        except:
            index().infoDialog("Trailer not found")
            return

    def trakt_search(self, url):
        try:
            result = client.request(url)
            result = re.search('href="((?:http|https)://[w\.]*youtube\.com/watch\?v=[a-zA-Z0-9_-]+)"', result).groups()[0]
    ...
    def youtube_search(self, name):
        try:
            ...
            url = '%s%s' % (self.youtube_query, query)
            result = client.request(url)
            result = re.search('href="(/watch\?v=[a-zA-Z0-9_-]+)"', result).groups()[0]

            url = '%s%s' % (self.youtube_base, result)
...
class resolver:
    def run(self, srcs, name, download=False):
        try:
            #srcs = self.tinkle(srcs, name)
            srcs = cache.get(self.tinkle, 1, srcs, name)
            if srcs is None: raise Exception()
            random.shuffle(srcs)
            totalSrcs = len(srcs)
            url = False
            ...
            if totalSrcs > 1:
                if getSetting("autoplay") == 'true':
                    ...
                    mirrors = []
                    for x in srcs:
                        mirrors.extend(['Mirror #%s (%s)' % (i, x[0])])
                        i += 1
                ...
                else: src = random.randrange(totalSrcs)
                
                pDialog = xbmcgui.DialogProgress()
                pDialog.create('AutoPlay Progess', 'Opening stream from %s...' % srcs[src][0])
            
                try:
                    #url = HostedMediaFile(url=srcs[src][1]).resolve()
                    url = self.source(srcs[src][1])
                    if not url: raise Exception()
                except:
                    del srcs[src]
                    totalSrcs -= 1
                    if not src == 0: src = 0
                    while not url and src < totalSrcs:
                        mirror = src+1
                        percent = int( ( (mirror) / float(totalSrcs) ) * 100)
                        message = "Trying %s, mirror %s out of %s remaining hosts." % (srcs[src][0], mirror, totalSrcs)
                        pDialog.update(percent, "The host selected failed to play.", message, "")
                        try:
                            #url = HostedMediaFile(url=srcs[src][1]).resolve()
                            url = self.source(srcs[src][1])
                            if not url: raise Exception()
    ...
    def source(self, url):
        ...
        url = client.replaceHTMLCodes(url)
        url = url.encode('utf-8')
        popups = True if getSetting("popups") == "true" else False
        url = HostedMediaFile(url=url).resolve(allow_popups=popups)
        
        return url
        
    def tinkle(self, url, name):
        #if url.startswith("//"): url = "http:%s" % url
        #url = url.replace("http://5movies.to/", "https://movie25.ag/")
        referer = url
        ...
        try:
            ...
            if imdb == None:
                try: imdb = re.findall('imdb.com/title/tt(\d+)', client.request(referer), re.I)[0]
                except: pass
            if imdb == None:
                try: imdb = json.loads(client.request('http://www.omdbapi.com/?t=%s&y=%s' % (urllib.quote_plus(title), str(int(year)))))['imdbID']
                except: pass
            if imdb == None:
                try: imdb = json.loads(client.request('http://www.omdbapi.com/?t=%s&y=%s' % (urllib.quote_plus(title), str(int(year)-1))))['imdbID']
                except: pass
    ...
    def movie25(self, imdb):
        ...
        try:
            ...
            result = client.request('http://www.omdbapi.com/?i=tt%s' % imdb)
            result = json.loads(result)
            title = result['Title']
            year = result['Year']
            ...
            result = client.parseDOM(result, "div", attrs = { "class": "movie_table" })[0]
            result = client.parseDOM(result, "li")

            match = [i for i in result if any(x in i for x in [' (%s)' % str(year), ' (%s)' % str(int(year)+1), ' (%s)' % str(int(year)-1)])]
            match2 = [self.base_link + client.parseDOM(i, "a", ret="href")[0] for i in match]
            if match2 == []: return
            for i in match2[:10]:
                try:
            ...
            result = client.parseDOM(match3, "div", attrs = { "class": "links_quality" })[0]
            links = client.parseDOM(result, "ul")
            for i in links:
                try:
                    name = client.parseDOM(i, "a")[0]
                    name = client.replaceHTMLCodes(name)
                    if name.isdigit(): raise Exception()
                    host = client.parseDOM(i, "li", attrs = { "class": "link_name" })[0]
        ...
        for i in movie25_sources:
            try:
                ...
                result = result.decode('iso-8859-1').encode('utf-8')
                url = client.parseDOM(result, "input", ret="onclick")
