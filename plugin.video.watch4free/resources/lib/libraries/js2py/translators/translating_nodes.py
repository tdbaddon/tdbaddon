...
def UnaryExpression(type, operator, argument, prefix):
    ...
    if operator=='delete':
        ...
        return js_typeof(a)
    result = UNARY[operator](a)
    result = strip_leading_parens(result)
    return result
...
def BinaryExpression(type, operator, left, right):
    ...
    result = BINARY[operator](a,b)
    result = strip_leading_parens(result)
    return result

def strip_leading_parens(expression):
    depth = 0
    match = re.search('^\(+', expression)
    if match:
        paren_count = len(match.group(0))
        if paren_count > MAX_STACK:
            new_exp = list(expression)
            for i, c in enumerate(new_exp[paren_count:]):
                if c == '(':
                    depth += 1
                elif c == ')':
                    if depth <= 0:
                        new_exp[paren_count+i]=''
                    else:
                        depth -= 1

            new_exp = ''.join(new_exp)[paren_count:]
            return new_exp
...
if __name__=='__main__':
    ...
    c = '''`ijfdij`'''
    if not c:
        with codecs.open("test.js", "r", "utf-8") as f:
            c = f.read()
