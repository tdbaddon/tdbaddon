...
import time
import datetime
from resources.lib.libraries.js2py.base import MakeError
...
def get_tz_dif():
    now = time.time()
    dif = datetime.datetime.fromtimestamp(now) - datetime.datetime.utcfromtimestamp(now)
    if dif.days < 0:
        return (datetime.datetime.utcfromtimestamp(now) - datetime.datetime.fromtimestamp(now)).seconds
    else:
        return (datetime.datetime.fromtimestamp(now) - datetime.datetime.utcfromtimestamp(now)).seconds
